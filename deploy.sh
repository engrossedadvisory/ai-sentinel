...
# ── Bring down existing containers before scanning ports ─────────────────
# This ensures our own running containers don't appear as "ports in use"
# during the scan, so ports stay stable across updates.
...
port_in_use() {
  ...
  # Docker port table — catches ports held by other containers
...
# Ports come from saved config above.
# If OLLAMA_HOST points to host.docker.internal, inject extra_hosts so the
# backend container can resolve it (required on Linux Docker).
OVERRIDE="${INSTALL_DIR}/docker-compose.override.yml"
ENV_FILE="${INSTALL_DIR}/.env"

OLLAMA_HOST_VAL="http://localhost:11434"
[[ -f "${ENV_FILE}" ]] && OLLAMA_HOST_VAL=$(grep -E '^OLLAMA_HOST=' "${ENV_FILE}" | cut -d= -f2- | tr -d '"' || echo "http://localhost:11434")

if echo "${OLLAMA_HOST_VAL}" | grep -q "host.docker.internal"; then
...
${EXTRA_HOSTS_BLOCK}
  frontend:
    ports:
      - "${FRONTEND_PORT}:80"
OVERRIDE_EOF
echo "    ✓ Override written → ${OVERRIDE}"
...
# For native Ollama on this host use: http://host.docker.internal:11434
# (deploy.sh will automatically add extra_hosts to the backend container)
